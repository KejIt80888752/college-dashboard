...
export default function Leaves() {
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      await api.put('/admin/leaves/action', {
        leave_id: id,
        action: action === 'approve' ? 'Approved' : 'Rejected',
      });
      // Update local state immediately
      setLeaves(prev => prev.map(l =>
        (l.leave_id === id || l.id === id)
  ...
  const filtered = filter === 'all' ? leaves
    : leaves.filter(l => l.status?.toLowerCase() === filter);

  return (
    <div style={{ padding: '28px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '800', color: '#111827', margin: 0 }}>Leave Requests</h1>
          <p style={{ fontSize: '13px', color: '#9CA3AF', marginTop: '4px' }}>Manage student leave applications</p>
        </div>
        <button onClick={fetchLeaves} disabled={loading}
          style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 16px', background: '#F3F4F6', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: '600', color: '#6B7280', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          { label: 'Total Requests', value: leaves.length, color: '#3D3BF3', bg: '#EEF0FF', Icon: ClipboardList },
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '20px' }}>
        {[
          ...
          <button key={key} onClick={() => setFilter(key)}
            style={{ padding: '8px 18px', borderRadius: '10px', border: 'none', fontSize: '13px', fontWeight: '600', cursor: 'pointer', background: filter === key ? '#3D3BF3' : '#F3F4F6', color: filter === key ? '#fff' : '#6B7280' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '64px', background: '#FFFFFF', borderRadius: '20px' }}>
          ...
          <p style={{ color: '#9CA3AF', marginTop: '12px' }}>Loading leave requests...</p>
        </div>
      ) : (
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <div key={id} style={{ background: '#FFFFFF', borderRadius: '16px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '14px', background: '#EEF0FF', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#3D3BF3', fontWeight: '800', fontSize: '18px', flexShrink: 0 }}>
                  {(l.student_name || 'S')[0].toUpperCase()}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                    <span style={{ fontSize: '15px', fontWeight: '700', color: '#111827' }}>{l.student_name || `Student #${l.student_id}`}</span>
                    {l.department && <span style={{ fontSize: '12px', color: '#9CA3AF' }}>· {l.department} Sem {l.semester}</span>}
                  </div>
                  {l.reason && <div style={{ fontSize: '13px', color: '#6B7280', marginBottom: '6px' }}>"{l.reason}"</div>}
                  <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                    {l.from_date && (
                      <span style={{ fontSize: '12px', color: '#9CA3AF', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Calendar size={12} /> {l.from_date} → {l.to_date} ({days(l.from_date, l.to_date)})
                      </span>
                    )}
                    {l.applied_on && <span style={{ fontSize: '12px', color: '#9CA3AF' }}>Applied: {l.applied_on}</span>}
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', fontWeight: '700', color: s.color, background: s.bg, padding: '5px 12px', borderRadius: '8px' }}>
                    <StatusIcon size={13} /> {s.label}
                  </span>
                  {['Pending', 'pending'].includes(status) && (
                    <>
                      <button onClick={() => handleAction(id, 'approve')} disabled={!!actionLoading}
                        style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 14px', borderRadius: '10px', border: 'none', background: '#F0FDF4', color: '#16A34A', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
                      ...
                      <button onClick={() => handleAction(id, 'reject')} disabled={!!actionLoading}
                        style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 14px', borderRadius: '10px', border: 'none', background: '#FEF2F2', color: '#DC2626', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
