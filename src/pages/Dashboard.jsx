    ...
    <div style={{ padding: '24px', minHeight: '100%' }}>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#9CA3AF', marginTop: '3px' }}>{today}</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '20px' }}>
        {cards.map(({ label, value, icon: Icon, color, bg }) => (
          <div key={label} style={{ background: '#FFFFFF', borderRadius: '18px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #F3F4F6' }}>
            ...
            <div style={{ fontSize: '13px', color: '#374151', marginTop: '3px', fontWeight: '500' }}>{label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '20px' }}>
        {quickStats.map(({ label, value, Icon, color, bg }) => (
          <div key={label} style={{ background: '#FFFFFF', borderRadius: '14px', padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', gap: '12px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
        ...
        {/* Upcoming Events */}
        <div style={{ background: '#FFFFFF', borderRadius: '18px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #F3F4F6' }}>
          <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#111827', margin: '0 0 14px' }}>Upcoming Events</h3>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '32px 16px', color: '#9CA3AF' }}>
              <Calendar size={32} color="#E5E7EB" style={{ display: 'block', margin: '0 auto 8px' }} />
                ...
                return (
                  <div key={id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', background: bg, border: `1px solid ${color}22` }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: '11px', color, fontWeight: '600', marginTop: '2px' }}>
                        {ev.event_date}{ev.venue ? ` · ${ev.venue}` : ''}
                      </div>
                    </div>
        ...
        {/* System Summary */}
        <div style={{ background: '#FFFFFF', borderRadius: '18px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #F3F4F6' }}>
          <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#111827', margin: '0 0 14px' }}>System Summary</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {summaryRows.map(({ label, value, color, bg, Icon }) => (
              <div key={label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 14px', borderRadius: '10px', background: bg }}>
          ...
          {/* Backend info */}
          <div style={{ marginTop: '16px', padding: '10px 14px', borderRadius: '10px', background: '#F9FAFB', border: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Wifi size={13} color="#22C55E" />
            <span style={{ fontSize: '12px', color: '#6B7280', fontWeight: '500' }}>Connected to backend · 13.53.193.80</span>
          </div>
        </div>
